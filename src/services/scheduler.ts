  ...
  for (const p of allProducts) {
    ...
    try {
      await db.insert(products).values({
        ...
        // Three-field pricing breakdown (Ajio always populates listed+promo;
        // bank is set by dealDetector during the scan if a PDP offer applies).
        listedPrice: p.listedPrice ?? null,
        promoDiscount: p.promoDiscount ?? null,
        bankDiscount: p.bankDiscount ?? null,
        ...
        set: {
          ...
          listedPrice: sql`excluded.listed_price`,
          promoDiscount: sql`excluded.promo_discount`,
          bankDiscount: sql`excluded.bank_discount`,
